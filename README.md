...
| 📝 Markdown 转图片 | 引流 / 内容发布类小工具 | 不需要（纯本地 Pillow 排版，仓库打包中文字体 wqy-microhei） |
| 🔐 文本加解密 | 趣味 / 工具类小工具 | 不需要（pycryptodome 本地 AES，Base64 免依赖） |
| 🎙️ 语音转文字 | 音频处理类小工具 | 不需要（faster-whisper 本地识别，首次下载 tiny 模型 ~40MB） |
| 📷 二维码解析 | 引流 / 工具类小工具 | 不需要（opencv 本地识别，与二维码生成对称） |
| 🌐 网页转图片截图 | 截图 / 配图类小工具 | 需要（playwright + 本地已装 chromium 浏览器；云端通常不可用） |
| 🧮 在线计算器集合 | 计算类小工具 | 不需要（纯 Python 计算：房贷/BMI/复利/分期/个税） |
